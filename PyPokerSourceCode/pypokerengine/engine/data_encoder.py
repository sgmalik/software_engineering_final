...
class DataEncoder:
  ...
  @classmethod
  def encode_player(cls, player, holecard=False):
    hash_ = {
        "name": player.name,
        "uuid": player.uuid,
        "stack": player.stack,
        "state": cls.__payinfo_to_str(player.pay_info.status)
        }
    if holecard:
  ...
  @classmethod
  def encode_seats(cls, seats):
    return {
        "seats": [cls.encode_player(player) for player in seats.players]
        }

  @classmethod
  def encode_pot(cls, players):
  ...
  @classmethod
  def encode_game_information(cls, config, seats):
    hsh = {
    ...
    hsh.update(cls.encode_seats(seats))
    return hsh

  @classmethod
  def encode_valid_actions(cls, call_amount, min_bet_amount, max_bet_amount):
    return {
        "valid_actions": [
  ...
  @classmethod
  def encode_action(cls, player, action, amount):
    return {
  ...
  @classmethod
  def encode_street(cls, street):
    return {
        "street": cls.__street_to_str(street)
        }

  @classmethod
  def encode_action_histories(cls, table):
    ...
    street_histories = [cls.__order_histories(table.sb_pos(), histories) for histories in street_histories]
  ...
  @classmethod
  def encode_winners(cls, winners):
    return { "winners": cls.__encode_players(winners) }

  @classmethod
  def encode_round_state(cls, state):
    hsh = {
        "street": cls.__street_to_str(state["street"]),
        "pot": cls.encode_pot(state["table"].seats.players),
    ...
    hsh.update(cls.encode_seats(state["table"].seats))
    hsh.update(cls.encode_action_histories(state["table"]))
    return hsh


  @classmethod
  def __payinfo_to_str(cls, status):
    if status == PayInfo.PAY_TILL_END:
      return cls.PAY_INFO_PAY_TILL_END_STR
    if status == PayInfo.ALLIN:
      return cls.PAY_INFO_ALLIN_STR
    if status == PayInfo.FOLDED:
      return cls.PAY_INFO_FOLDED_STR

  @classmethod
  def __street_to_str(cls, street):
  ...
  @classmethod
  def __encode_players(cls, players):
    return [cls.encode_player(player) for player in players]

  @classmethod
  def __order_histories(cls, start_pos, player_histories):
    ...
    unified_histories = [cls.__unify_length(max_len, l) for l in all_player_histories]
  ...
  @classmethod
  def __unify_length(cls, max_len, lst):
