...
class Card:
    ...
    def draw(self, screen):
        ...
        if not self.revealed:
            ...
            # Draw the rank sprite (bottom-right)
            bottom_right_rank_x = self.position[0] + 13 * self.scale[0]  # 13 pixels from the left (scaled)
            bottom_right_rank_y = self.position[1] + 23 * self.scale[1]  # 23 pixels from the top (scaled)
            screen.blit(self.rank_sprite, (bottom_right_rank_x, bottom_right_rank_y))
